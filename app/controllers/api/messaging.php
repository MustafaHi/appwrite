...
Http::post('/v1/messaging/providers/gowa')
    ->desc('Create GoWA provider')
    ->groups(['api', 'messaging'])
    ->label('audits.event', 'provider.create')
    ->label('audits.resource', 'provider/{response.$id}')
    ->label('event', 'providers.[providerId].create')
    ...
    ->label('sdk', new Method(
        namespace: 'messaging',
        group: 'providers',
        name: 'createGowaProvider',
        description: '/docs/references/messaging/create-gowa-provider.md',
        auth: [AuthType::ADMIN, AuthType::KEY],
        responses: [
            new SDKResponse(
                code: Response::STATUS_CODE_CREATED,
    ...
    ->param('providerId', '', fn (Database $dbForProject) => new CustomId(false, $dbForProject->getAdapter()->getMaxUIDLength()), 'Provider ID. Choose a custom ID or generate a random ID with `ID.unique()`. Valid chars are a-z, A-Z, 0-9, period, hyphen, and underscore. Can\'t start with a special char. Max length is 36 chars.', false, ['dbForProject'])
    ->param('name', '', new Text(128), 'Provider name.')
    ->param('from', '', new Text(0), 'Sender Phone number. Format this number with a leading \'+\' and a country code, e.g., +16175551212.', true)
    ->param('url', '', new Text(0), 'GoWA service URL.', true)
    ->param('token', '', new Text(0), 'GoWA service token.', true)
    ->param('enabled', null, new Nullable(new Boolean()), 'Set as enabled.', true)
    ->inject('queueForEvents')
    ->inject('dbForProject')
    ->inject('response')
    ->action(function (string $providerId, string $name, string $from, string $url, string $token, ?bool $enabled, Event $queueForEvents, Database $dbForProject, Response $response) {
        $providerId = $providerId == 'unique()' ? ID::unique() : $providerId;
        ...
        if (
            $enabled === true
            && \array_key_exists('url', $credentials)
            && \array_key_exists('token', $credentials)
            && \array_key_exists('from', $options)
        ) {
            $enabled = true;
        } else {
...
Http::patch('/v1/messaging/providers/gowa/:providerId')
    ->desc('Update GoWA provider')
    ->groups(['api', 'messaging'])
    ->label('audits.event', 'provider.update')
    ->label('audits.resource', 'provider/{response.$id}')
    ->label('event', 'providers.[providerId].update')
    ...
    ->label('sdk', new Method(
        namespace: 'messaging',
        group: 'providers',
        name: 'updateGowaProvider',
        description: '/docs/references/messaging/update-gowa-provider.md',
        auth: [AuthType::ADMIN, AuthType::KEY],
        responses: [
            new SDKResponse(
                code: Response::STATUS_CODE_OK,
    ...
    ->inject('dbForProject')
    ->inject('response')
    ->action(function (string $providerId, string $name, ?bool $enabled, string $url, string $token, string $from, Event $queueForEvents, Database $dbForProject, Response $response) {
        $provider = $dbForProject->getDocument('providers', $providerId);
